...
class AccountWhSrc(models.Model):
    ...
    _description = "SRC Withholding Voucher"
    ...
    journal_id = fields.Many2one(
        ...
        default=lambda self: self.env.company.wh_src_journal_id,
    )
    ...
    def _onchange_move_id(self):
        if self.move_id:
            ...
            self.wh_rate = self.move_id.partner_id.wh_src_rate or self.wh_rate

    def action_confirm(self):
        for rec in self:
            if rec.name == "/" or not rec.name:
                rec.name = self.env["ir.sequence"].next_by_code("account.wh.src") or rec.id
            if rec.amount_total_ret <= 0:
                raise ValidationError(_("SRC withholding amount must be greater than zero."))
            account = rec.company_id.wh_src_account_id
            if rec.payment_id:
                rec.state = "done"
                if rec.move_id:
                    rec.move_id.wh_src_id = rec.id
            ...
            if rec.journal_id and account and not rec.accounting_move_id:
                move = self.env["account.move"].create({
                    ...
                    "line_ids": [
                        (0, 0, {
                            "name": _("SRC withholding %s") % rec.name,
