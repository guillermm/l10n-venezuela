...
class IslrWhRates(models.Model):
    ...
    code = fields.Char(
        ...
        required=True,
        size=3,
        help="Official 3-digit concept code for this person type (XML CodigoConcepto).",
    )
    ...
    _sql_constraints = [
        ("code_unique", "unique(code)", "Each SENIAT ISLR concept code must be unique."),
        (
            "person_type_uniq",
            "unique(concept_id, person_type)",
            "A concept can have only one rate per person type.",
        ),
    ]

    def name_get(self):
        return [
            (rec.id, "%s — %s" % (rec.code, rec.person_type.upper() if rec.person_type else rec.display_name))
            for rec in self
        ]
