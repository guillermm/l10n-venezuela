...
class AccountWhIvaLineTax(models.Model):
    ...
    is_exempt = fields.Boolean(string="Exempt / SDCF")
    amount_ret = fields.Float(string="Withheld Amount", compute="_compute_ret", store=True)

    @api.depends("tax_amount", "wh_rate", "is_exempt")
    def _compute_ret(self):
        for rec in self:
            rec.amount_ret = 0.0 if rec.is_exempt else rec.tax_amount * (rec.wh_rate / 100.0)
