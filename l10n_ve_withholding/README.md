...
Shared architecture for Venezuelan withholdings (IVA, ISLR, Municipal, SRC) on Odoo 17: invoice withholding status and payment-register write-off helpers so each engine applies the retention when registering a payment.
...
Install together with the engines you need (`l10n_ve_withholding_iva`, `islr`, `muni`, `src`). Amounts are applied in *Accounting -> Register Payment*.
...
This module is part of the [OCA/l10n-venezuela](https://github.com/OCA/l10n-venezuela/tree/17.0/l10n_ve_withholding) project on GitHub.
